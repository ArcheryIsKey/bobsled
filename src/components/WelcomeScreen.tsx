...
import { User, Flask } from '@phosphor-icons/react';
...
export default function WelcomeScreen({
 ...
 <div className="bg-background border border-white/10 p-8 sm:p-10 rounded-3xl shadow-2xl flex flex-col items-center text-center gap-6 relative overflow-hidden">
 ...
 bobsled<span className="text-primary">.</span>gg
 </h1>
 ...
 className="w-full h-12 bg-primary hover:bg-red-600 active:scale-[0.99] text-white font-bold text-xs uppercase tracking-wider rounded-full transition-all shadow-[0_0_20px_rgba(255,77,77,0.4)] flex items-center justify-center gap-2 cursor-pointer"
 >
 <User size={16} />
 ...
 <span className="absolute bg-background px-3 text-xs text-text-muted uppercase">OR</span>
 </div>
 ...
 className="w-full h-11 bg-black border border-white/10 focus:border-primary rounded-full px-4 text-xs text-white text-center outline-none transition-colors placeholder:text-text-muted"
 />
 </div>
 <button
 type="submit"
 disabled={!guestUsername.trim()}
 className="w-full h-10 bg-[#202020] hover:bg-[#282828] disabled:opacity-40 text-white font-semibold text-xs uppercase tracking-wider rounded-full transition-all border border-white/10 flex items-center justify-center gap-1.5 cursor-pointer"
 >
 <Flask size={13} className="text-primary"/>

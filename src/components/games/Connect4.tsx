...
export default function Connect4({ game, user, isSpectator, onMove }: Connect4Props) {
 ...
 {game?.status === 'active' ? (
 isOffline ? (
 <div className="bg-red-950/40 border border-red-500/40 text-red-300 px-4 sm:px-5 py-1.5 rounded-full text-xs sm:text-xs tracking-wider flex items-center justify-center gap-2 shadow-md whitespace-nowrap leading-none">
 <WifiSlash size={14} className="text-red-400 animate-pulse"/>
 <span>Offline — Moves Paused</span>
 </div>
 ) : isMovePending ? (
 <div className="bg-[#1e1e1e] border border-primary/40 text-primary px-4 sm:px-5 py-1.5 rounded-full text-xs sm:text-xs tracking-wider flex items-center justify-center gap-2 shadow-md whitespace-nowrap leading-none">
 <CircleNotch size={14} className="animate-spin text-primary shrink-0"/>
 <span>Submitting Move...</span>
 </div>
 ) : isSpectator ? (
 <div className="bg-[#1e1e1e] border border-white/10 text-text-secondary px-4 sm:px-5 py-1.5 rounded-full text-xs sm:text-xs tracking-wider flex items-center justify-center gap-2 shadow-sm leading-none">
 ...
 initial={{ scale: 0.95 }}
 animate={{ scale: 1 }}
 className={`border px-4 sm:px-5 py-1.5 rounded-full text-xs sm:text-xs tracking-wider flex items-center justify-center gap-2 font-bold uppercase whitespace-nowrap shadow-md leading-none ${
 myDiscIsRed
 ...
 <div className="bg-[#1a1a1a] border border-white/10 text-text-muted px-4 sm:px-5 py-1.5 rounded-full text-xs sm:text-xs tracking-wider flex items-center justify-center gap-2 font-semibold uppercase shadow-sm whitespace-nowrap leading-none">
 <span className="w-2 h-2 bg-text-muted rounded-full animate-pulse shrink-0"/>
 <span>Opponent's Turn ({myDiscIsRed ? 'White' : 'Red'})</span>
 </div>
 )
 ) : game?.status === 'waiting' ? (
 <div className="bg-[#1e1e1e] border border-primary/40 text-primary px-4 sm:px-5 py-1.5 rounded-full text-xs sm:text-xs tracking-wider flex items-center justify-center gap-2 font-semibold uppercase shadow-md whitespace-nowrap leading-none">
 ...
 className="bg-[#1e1e1e] border border-primary/60 text-primary px-4 sm:px-5 py-1.5 rounded-full text-xs sm:text-xs tracking-wider flex items-center justify-center gap-2 font-bold uppercase shadow-[0_0_20px_rgba(255,77,77,0.4)] whitespace-nowrap leading-none"
 >
 ...
 className="w-11 h-11 min-[370px]:w-12 min-[370px]:h-12 min-[410px]:w-14 min-[410px]:h-14 sm:w-16 sm:h-16 md:w-[72px] md:h-[72px] rounded-full flex items-center justify-center relative bg-[#0c0c0c] shadow-[inset_0_3px_8px_rgba(0,0,0,0.9),0_1px_1px_rgba(255,255,255,0.05)] border border-white/5 overflow-visible"
 >

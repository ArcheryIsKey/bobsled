...
export class ErrorBoundary extends ComponentBase {
 ...
 className="flex-1 min-w-[140px] px-5 py-2.5 rounded-full bg-primary hover:bg-red-600 text-white font-bold text-xs uppercase tracking-wider transition-all shadow-[0_0_20px_rgba(255,77,77,0.35)] flex items-center justify-center gap-2 cursor-pointer hover:scale-[1.02] active:scale-[0.98]"
 >
 ...
 className="flex-1 min-w-[140px] px-5 py-2.5 rounded-full bg-white/10 hover:bg-white/15 border border-white/15 text-white font-bold text-xs uppercase tracking-wider transition-all flex items-center justify-center gap-2 cursor-pointer hover:scale-[1.02] active:scale-[0.98]"
 >
 ...
 className="w-full flex items-center justify-between text-xs text-text-muted hover:text-text-secondary cursor-pointer transition-colors py-1"
 >
 ...
 <div className="mt-3 bg-black/70 border border-white/10 rounded-xl p-3 relative text-xs leading-relaxed text-red-300 overflow-x-auto max-h-48 scrollbar-thin">
 <button
 type="button"
 onClick={this.handleCopyDetails}
 className="absolute top-2 right-2 px-2 py-1 bg-white/10 hover:bg-white/20 rounded text-[12px] text-white flex items-center gap-1 transition-colors cursor-pointer"
 >
 ...
 {error?.stack && (
 <pre className="text-[12px] text-text-muted whitespace-pre-wrap break-all mt-1 font-mono">
 {error.stack}
 </pre>
 )}
 {errorInfo?.componentStack && (
 <pre className="text-[12px] text-text-muted whitespace-pre-wrap break-all mt-2 border-t border-white/10 pt-2 font-mono">
 ...
 <p className="text-xs text-text-secondary mb-5 max-w-sm">
 ...
 className="px-4 py-2 rounded-full bg-primary hover:bg-red-600 text-white font-bold text-xs uppercase tracking-wider transition-all flex items-center gap-1.5 cursor-pointer"
 >
 ...
 type="button"
 onClick={this.handleGoHome}
 className="px-4 py-2 rounded-full bg-white/10 hover:bg-white/15 border border-white/10 text-white font-bold text-xs uppercase tracking-wider transition-all flex items-center gap-1.5 cursor-pointer"
 >
 ...
 type="button"
 onClick={this.toggleDetails}
 className="w-full flex items-center justify-between text-xs text-text-muted hover:text-text-secondary cursor-pointer"
 >
 <span>Error Details: {error?.message?.slice(0, 40)}...</span>
 {showDetailsAccordion ? <CaretUp size={12} /> : <CaretDown size={12} />}
 </button>
 {showDetailsAccordion && (
 <div className="mt-2 bg-black/80 border border-white/10 rounded-lg p-2.5 text-[12px] text-red-300 relative max-h-36 overflow-y-auto">
 <button
 type="button"
 onClick={this.handleCopyDetails}
 className="absolute top-1.5 right-1.5 px-1.5 py-0.5 bg-white/10 hover:bg-white/20 rounded text-[11px] text-white flex items-center gap-1"
 >
 {copied ? <Check size={10} className="text-green-400"/> : <Copy size={10} />}
 <span>{copied ? 'Copied' : 'Copy'}</span>
 </button>
 <div className="font-bold text-white">{error?.name}: {error?.message}</div>
 {error?.stack && <pre className="text-text-muted whitespace-pre-wrap mt-1 font-mono">{error.stack}</pre>}
 </div>
 )}
 </div>
 </div>
 ...
 return (
 <div className="w-full p-4 rounded-xl bg-red-950/20 border border-red-500/20 text-text-primary flex flex-col gap-2 my-2 text-xs">
 ...
 type="button"
 onClick={this.handleReset}
 className="px-2.5 py-1 rounded bg-white/10 hover:bg-white/20 text-xs font-bold text-white transition-colors cursor-pointer"
 >
 Retry
 </button>
 </div>
